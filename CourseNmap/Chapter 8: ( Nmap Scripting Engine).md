...
# Как работает NSE
```bash
1. Nmap сначала находит открытые порты
↓
2. Определяет сервис
↓
3. Только после этого запускает подходящий скрипт
↓
4. Скрипт общается с сервисом на уровне приложения
```
...
# NSE 
NSE использует обычное TCP-соединение, но после TCP Handshake начинает обмениваться данными на уровне приложения (HTTP, SSH, DNS и т.д.):
```bash
TCP
↓
SYN
...
# Вывод 
NSE не ограничивается определением открытых портов, после установки TCP-соединения,  скрипт начинает общаться с сервисом (HTTP, SSH, DNS и др.) по его собственному протоколу и получает дополнительную информацию
