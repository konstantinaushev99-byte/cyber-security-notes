...
3. В результате команды увидел это: <img width="539" height="63" alt="image" src="https://github.com/user-attachments/assets/9a43a69c-5d67-4ddc-87ac-3887ec8a782f" />
Тут я увидел Программу: OpenSSH, Версию: 10.0, OC: Debian, и протокол: 2.0
4. Теперь вернемся в Wireshark, и заметим что сначало пошло подключение на проверку порта, после того как nmap проверил хост что он отвечает,
он установил полсное подключение, и ssh рассказал кто он и что он, после этого мы видим FIN,ACK - это значит что nmap перестал спрашиват ssh и хочет завершить подключение
